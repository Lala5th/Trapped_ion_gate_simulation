...
def run_templated_sim(data):
    ...
    for i,d in enumerate(data['sequence']):
        ...
        params["ts"] = np.linspace(d["abstime"]*(1 - data.get('t_r',0.1)),d["abstime"]*(1 + data.get('t_r',0.1)),data['n_t'])
        params["ts"] = np.insert(params["ts"],0,0)
...
if __name__ == "__main__":
    ...
    if(data['output']):
        ...
        data['params'].append({'label':"$\Delta t [t_g]$",'value':np.linspace(-data["template"].get('t_r',0.1),data["template"].get('t_r',0.1),data["template"]['n_t'])})
