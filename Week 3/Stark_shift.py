...
# Stark shift
Omega0_Stark = 8e1
Omegas_Stark = np.array([[0,Omega0_Stark],[np.conj(Omega0_Stark),0]], dtype=np.complex128)
omega_Stark = omega0 - Omega0_Stark*50

# Detuning
deltas = lambda omega : omega - np.abs(omegas)
sums = lambda omega : np.abs(abs(omega) + np.abs(omegas))

# Set up Hamiltonian
def Rabi_RWA_Stark(t):
    Hamiltonian = hbar*Omegas_Stark/2
    ds = omega_Stark - omega0
    Hamiltonian[0,1] *= np.exp( 1j*ds*t)# + np.exp(-1j*sums[0,0,0,1,1,1]*t)
    Hamiltonian[1,0] *= np.exp(-1j*ds*t)# + np.exp( 1j*sums[1,1,1,0,0,0]*t)
    return Hamiltonian
