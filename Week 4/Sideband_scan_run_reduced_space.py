...
for i in range(n_num-1):
    a[i,i+1] = np.sqrt(state_start + i)
    a_dagger[i+1,i] = np.sqrt(state_start + i)
...
if __name__ == '__main__':
    ...
    metadata = [n_num,1,nu0,Omega0]
    np.savez(f'O{Omega0}_nu{nu0}_eta0{(omega0 + nu0)*z_0/c}_Reduced_LDR',os = os, ts = ts, s3d = s3d, metadata = metadata)
    # for o in os:
    #     progr = -1000
    #     # Omega = np.sqrt((1 + (np.min(np.abs(o - sidebands)))**2)*abs(mod_Rabi_freq(omega0 + o*Omega0))**2)
    #     # Omega = mod_Rabi_freq(omega0 + o*Omega0)
