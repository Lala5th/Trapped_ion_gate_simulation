...
%    \end{frame}
	...
	\begin{frame}{Energy structure}
		\begin{figure}
			\centering
			\only<1>{\includegraphics[width=0.75\linewidth]{1ion_Elevels.pdf}}
	...
	\begin{frame}{M\o lmer-S\o rensen gate\footfullcite{MS-gate}}
		\centering
		\only<1>{\includegraphics[height=12em]{MS_Gate_Schematic.pdf}}
		\only<2>{\includegraphics[height=12em]{MS_gate_seq_OR.pdf}}
	...
	\begin{frame}{\only<1>{Qubit frequency error}\only<2->{Dynamical decoupling\footfullcite{DD}}}
		\begin{tabular*}{\linewidth}{c @{\extracolsep{\fill}}*3 c}
			\includegraphics[width=0.7\textheight]{qubit_err.pdf} &%
			%\begin{tabular}{c}
			\parbox[b][0.7\textheight][c]{0.7\textwidth}{
				\begin{align*}%
					\onslide<1->{\hat{H}_I &= \hat{H}_G - \sum_i^n\frac{\hbar\xi_i}{2}\hat{\sigma}_z^{(i)}}\\%
					\onslide<2->{\hat{H}_I &= \hat{H}_G - \sum_i^n\frac{\hbar\xi_i}{2}\hat{\sigma}_z^{(i)} + \frac{\hbar\Omega_c}{2}\sum_i^n\hat{\sigma}_y^{(i)}}\\%
					\onslide<3->{\hat{H}_I^\prime &= \hat{H}_G - \sum_i^n\frac{\hbar\xi_i}{2}\left(\hat{\sigma}_z^{(i)}\cos\left(\Omega_ct\right) + \hat{\sigma}_x^{(i)}\sin\left(\Omega_ct\right)\right)}
				\end{align*}
				\onslide<3->{\[\left[\hat{H}_G, \hat{\sigma}_y^{(i)}\right]=0\hspace*{2em} \Omega_c \gg \xi_i\hspace*{2em}\Omega_c \gg \frac{1}{t_g}\hspace*{2em}\]}
			}%
			%\end{tabular}
		\end{tabular*}
	\end{frame}
